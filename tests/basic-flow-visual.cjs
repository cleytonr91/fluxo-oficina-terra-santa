...
for(const modal of [false,true]){
 let state=0;const values=['2026-09-25',vehicles,'',true,true,'','',modal?vehicles[0]:null,modal?form:null,false,''];
 const {BasicFlowBoard}=load('src/components/basic-flow-board.tsx',{
  ...
  '@/context/auth-context':{useAuth:()=>({profile:{role:'admin',name:'Usuário de teste'},user:{uid:'test'},logout(){}})},
 ...
 const css=fs.readFileSync('src/components/basic-flow-board.module.css','utf8');
